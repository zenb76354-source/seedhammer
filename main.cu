...
int main(int argc, char **argv) {
    ...
    if(scan_mode) {
        ...
        // Try to load targets from vaultwatch
        uint32_t n_patoshi = 0;
        uint8_t *patoshi_h160s = NULL;
        FILE *pf = fopen("../vaultwatch/patoshi_h160.bin", "rb");
        if(pf) {
            fseek(pf, 0, SEEK_END); long sz = ftell(pf); rewind(pf);
            n_patoshi = sz / 20;
            patoshi_h160s = (uint8_t*)malloc(sz);
            fread(patoshi_h160s, 1, sz, pf); fclose(pf);
            printf("Loaded %u Patoshi targets from vaultwatch\n", n_patoshi);
        } else {
            // Fallback to internal targets
            n_patoshi = 8;
            patoshi_h160s = (uint8_t*)malloc(8*20);
            ...
            printf("Using 8 internal Patoshi targets\n");
        }
        ...
        for(uint32_t i=0; i<n_patoshi; i++){
            ...
            uint32_t h7[7] = {
                ...
                ((h[0]*2654435761u + h[1]*2246822519u + h[2]) & mask),
                ((h[3]*3266489917u + h[4]*668265263u + h[5]) & mask)
            };
